  ...
  const handleSaveAsTemplate = useCallback(async () => {
    ...
    try {
      await templates.create({
        name: editState.label || editState.id,
        description: (editState as { readme?: string }).readme || undefined,
        nodeType: editState.type,
        icon: spec?.icon,
        category: spec?.category,
        config: (editState.config || {}) as Record<string, unknown>,
      });
      queryClient.invalidateQueries({ queryKey: ['templates'] });
      toast.success('Saved as template');
    } catch (err) {
      toast.error(`Failed to save template: ${err instanceof Error ? err.message : String(err)}`);
    }
  ...
  return (
    <div className="w-full h-full bg-surface flex flex-col min-h-0 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 min-h-0">
        ...
        {/* Stage description (stored as `readme` on the stage) */}
